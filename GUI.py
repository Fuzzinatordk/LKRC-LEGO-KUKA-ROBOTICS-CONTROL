...
class RobotMovementGUI:
    def __init__(self, root):
        ...
        self.chosen_joint_config = None  # Attribute to store the chosen joint configuration
        ...
        self.teach_button = ttk.Button(self.frame, text="Teach movement", command=lambda: self.thread_function(self.teach))
        self.teach_button.grid(row=15, column=0, columnspan=2, sticky=(tk.W, tk.E))
    ...
    def teach(self):
        # Use the teach method of the robot and update the GUI with the result
        taught = self.robot.teach()
        if taught:
            # Assuming the `teach()` method returns the joint configuration as a list
            self.chosen_joint_config = taught
            print(f"Chosen Joint Configuration: {self.chosen_joint_config}")
        else:
            print("Teaching process was not successful.")
        
    def handle_result(self, result):
        # Implement this method to handle the result of a function call
        pass
